...
hmc_sqlite3 **hmc_trace_init = &hmc_sqlite3_data;
...
hmc_sqlite3::hmc_sqlite3(const char *dbname)
{
  ...
  sqlite3_stmt *query;
  const char *squery = "CREATE TABLE hmcsim_stat (cycle INTEGER, physicalPktAddr INTEGER, pktTag INTEGER, pktAddr INTEGER, typeId INTEGER, fromID INTEGER, toId INTEGER);";
  ...
  const char *s_sql_rqst = "INSERT INTO hmcsim_stat (cycle, physicalPktAddr, pktTag, pktAddr, typeId, fromId, toId) values (?1, ?2, ?3, ?4, ?5, ?6, ?7);";
  sqlite3_prepare_v2(this->db, s_sql_rqst, -1, &this->sql_rqst, nullptr);

  const char *s_sql_rsp = "INSERT INTO hmcsim_stat (cycle, physicalPktAddr, pktTag, typeId, fromId, toId) values (?1, ?2, ?3, ?4, ?5, ?6);";
                           ...
                           uint64_t pktAddr, unsigned typeId, int fromId, int toId)
{
  ...
  sqlite3_bind_int64(hmc_sqlite3_data->sql_rqst, 2, phyPktAddr);
  sqlite3_bind_int(hmc_sqlite3_data->sql_rqst, 3, pktTag);
  sqlite3_bind_int64(hmc_sqlite3_data->sql_rqst, 4, pktAddr);
  sqlite3_bind_int(hmc_sqlite3_data->sql_rqst, 5, typeId);
  sqlite3_bind_int(hmc_sqlite3_data->sql_rqst, 6, fromId);
  sqlite3_bind_int(hmc_sqlite3_data->sql_rqst, 7, toId);
...
void hmc_trace::trace_rsp(uint64_t cycle, uint64_t phyPktAddr, unsigned pktTag,
                          unsigned typeId, int fromId, int toId)
{
  sqlite3_bind_int64(hmc_sqlite3_data->sql_rsp, 1, cycle);
  sqlite3_bind_int64(hmc_sqlite3_data->sql_rsp, 2, phyPktAddr);
  sqlite3_bind_int(hmc_sqlite3_data->sql_rsp, 3, pktTag);
  sqlite3_bind_int(hmc_sqlite3_data->sql_rsp, 4, typeId);
  sqlite3_bind_int(hmc_sqlite3_data->sql_rsp, 5, fromId);
  sqlite3_bind_int(hmc_sqlite3_data->sql_rsp, 6, toId);
